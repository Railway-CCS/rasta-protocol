...
void generateBlake2(unsigned char* data, int data_length, const unsigned char * key, int key_length,
    ...
    if (blake2b_init(&blake_ctx, (size_t) hash_len, key, (size_t) key_length)){
        // something went wrong
        return;
    }

    blake2b_update(&blake_ctx, data, (size_t) data_length);
    blake2b_final(&blake_ctx, result);
}
